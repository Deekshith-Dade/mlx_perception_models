...
from core.tokenizer import TIKTOKEN_MAX_ENCODE_CHARS, TikTokenTokenizer, Tokenizer
...
class Llama3Tokenizer(TikTokenTokenizer):
    ...
    def encode(self, s: str, add_bos: bool, add_eos: bool) -> List[int]:
        ...
        MAX_NO_WHITESPACE_CHARS = 250_000
...
class PLMTokenizer(Llama3Tokenizer):
    ...
    def __call__(
        ...
        if media_type in ["image", "multi_image", "video"]:
            ...
            width, height = media.size(-2), media.size(-3)
            num_patches = int(
    ...
    def decode_batch(self, tokens: mx.array) -> List[str]:
